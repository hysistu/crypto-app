...
import {
  addTicketCategory,
  getOneTicketCategory,
  updateITicketCategory,
} from "requests/ticketCategory copy";
...
const TicketCategoriesItem: NextPage = () => {
  ...
  const invokeData = useCallback(async (id: string) => {
    const _dep: any = await getOneTicketCategory(id);
  ...
  return (
    <Layout title="Ticket Category Item">
      <GeneralForm
        model={data}
        addAction={addTicketCategory}
        actionUpdate={updateITicketCategory}
      />
    </Layout>
  );
};
